...
def test_get_products_sorted_by_date():
    ...
    try:
        dates = [datetime.strptime(product["last_modified"], '%Y-%m-%d') for product in products]
    except ValueError:
...
def test_get_products_sorted_by_rate():
    ...
    assert all(rates[index] >= rates[index + 1] for index in range(len(rates) - 1))
...
def test_get_products_sorted_by_name():
    ...
    assert all(sorted_list[index] <= sorted_list[index + 1] for index in range(len(sorted_list) - 1))
